...
docker_verify_minimum_env() {
    ...
    # 数据同步用户账户配置项检测 --begin--
    if [ -z "$MYSQL_SLAVE_USER" ]; then
        mysql_error "missing MYSQL_SLAVE_USER"
    fi

    if [ "$MYSQL_SLAVE_USER" = "root" ]; then
        mysql_error "MYSQL_SLAVE_USER can not be root"
    fi

    if [ -z "$MYSQL_SLAVE_PASSWORD" ]; then
        mysql_error "missing MYSQL_SLAVE_PASSWORD"
    fi
    # 数据同步用户账户配置项检测 --end--
...
docker_setup_env() {
    ...
    # 数据同步用户账户配置项 --begin--
    file_env 'MYSQL_SLAVE_USER'
    file_env 'MYSQL_SLAVE_PASSWORD'
    # 数据同步用户账户配置项 --end--
...
docker_setup_db() {
    ...
    # 数据同步用户账户创建及授权 --begin--
    # 参考https://www.modb.pro/db/29919
    if [ -n "$MYSQL_SLAVE_USER" ] && [ -n "$MYSQL_SLAVE_PASSWORD" ]; then
        mysql_note "Creating user ${MYSQL_SLAVE_USER}"
        docker_process_sql --database=mysql <<<"CREATE USER '$MYSQL_SLAVE_USER'@'%' IDENTIFIED WITH mysql_native_password BY '$MYSQL_SLAVE_PASSWORD' ;"

        mysql_note "Giving user ${MYSQL_SLAVE_USER} access with REPLICATION SLAVE and REPLICATION CLIENT"
        docker_process_sql --database=mysql <<<"GRANT REPLICATION SLAVE, REPLICATION CLIENT ON *.* TO '$MYSQL_SLAVE_USER'@'%' ;"
    fi
    # 数据同步用户账户创建及授权 --begin--
}
